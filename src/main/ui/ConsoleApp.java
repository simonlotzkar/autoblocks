...
public class AutoBlocksConsole {
    ...
    private List<NPC> encounter = new ArrayList<>();

    // selections
    private StatBlock selectedStatBlock;
    private NPC selectedNPC;
    private String selectedGroupName;
    private List<NPC> selectedGroup;
    ...
    private void displayStats() {
        if (selectedGroupName != null) {
            for (NPC c : selectedGroup) {
                displayIndividualStats(c);
            }
        } else if (selectedNPC != null) {
            displayIndividualStats(selectedNPC);
        } else {
    ...
    private void displayIndividualAbilityScores(StatBlock selected) {
        AbilityScoreSet abilityScoreSet = selected.getAbilityScores();
        System.out.println("\tStr: " + abilityScoreSet.getStrength()
                + "(" + abilityScoreSet.toModifier("strength") + ")"
                + ", Dex: " + abilityScoreSet.getDexterity()
                + "(" + abilityScoreSet.toModifier("dexterity") + ")"
                + ", Con: " + abilityScoreSet.getConstitution()
                + "(" + abilityScoreSet.toModifier("constitution") + ")"
                + ", Int: " + abilityScoreSet.getIntelligence()
                + "(" + abilityScoreSet.toModifier("intelligence") + ")"
                + ", Wis: " + abilityScoreSet.getWisdom()
                + "(" + abilityScoreSet.toModifier("wisdom") + ")"
                + ", Cha: " + abilityScoreSet.getCharisma()
                + "(" + abilityScoreSet.toModifier("charisma") + ")");
    }
    ...
    private void displayIndividualActions(StatBlock selected) {
        ...
        for (RollableAction a : selected.getActions()) {
    ...
    private void editHP() {
        if (selectedNPC != null) {
            ...
            selectedNPC.changeHP(hpDifference);
        } else {
    ...
    private void changeCharacterGroup() {
        if (selectedNPC.getTitle().getGroup() != null) {
            ...
            if (newGroup.equals("rem")) {
                selectedNPC.getTitle().setGroup(null);
                System.out.println("Removed this character's group.");
            } else {
                selectedNPC.getTitle().setGroup(newGroup.substring(0, 1).toUpperCase() + newGroup.substring(1));
    ...
    private void displayRollMenuActions() {
        if (selectedNPC != null) {
            displayIndividualActions(selectedNPC);
        } else {
    ...
    private void processActionCommand(String command) {
        if (selectedNPC != null) {
            rollCharacterActionByName(selectedNPC, command);
        } else {
            for (NPC c : selectedGroup) {
    ...
    private void rollCharacterActionByName(NPC NPC, String action) {
        for (RollableAction a : NPC.getActions()) {
            if (a.getName().equalsIgnoreCase(action)) {
                System.out.println(NPC.getTitle().getName() + "'s " + a.generateFullRollString());
                break;
            }
        }
    }
    ...
    private void displayAbilityCheck(String ability) {
        if (selectedNPC != null) {
            System.out.println(selectedNPC.getTitle().getName()
                    + "'s " + ability + "check: " + rollAbilityCheck(selectedNPC, ability) + ".");
        } else {
            for (NPC c : selectedGroup) {
    ...
    private int rollAbilityCheck(NPC NPC, String ability) {
        switch (ability) {
            case "strength":
                return rollStrengthCheck(NPC);
            case "dexterity":
                return rollDexterityCheck(NPC);
            case "constitution":
                return rollConstitutionCheck(NPC);
            case "intelligence":
                return rollIntelligenceCheck(NPC);
            case "wisdom":
                return rollWisdomCheck(NPC);
            default:
                return rollCharismaCheck(NPC);
        }
    }

    // EFFECTS: rolls a strength check for the given character
    private int rollStrengthCheck(NPC NPC) {
        RollFormula rollFormula = new RollFormula(1, 20, NPC.getAbilityScores().toModifier("strength"));
        return rollFormula.roll();
    }

    // EFFECTS: rolls a dexterity check for the given character
    private int rollDexterityCheck(NPC NPC) {
        RollFormula rollFormula = new RollFormula(1, 20, NPC.getAbilityScores().toModifier("dexterity"));
        return rollFormula.roll();
    }

    // EFFECTS: rolls a constitution check for the given character
    private int rollConstitutionCheck(NPC NPC) {
        RollFormula rollFormula = new RollFormula(1, 20, NPC.getAbilityScores().toModifier("constitution"));
        return rollFormula.roll();
    }

    // EFFECTS: rolls a intelligence check for the given character
    private int rollIntelligenceCheck(NPC NPC) {
        RollFormula rollFormula = new RollFormula(1, 20, NPC.getAbilityScores().toModifier("intelligence"));
        return rollFormula.roll();
    }

    // EFFECTS: rolls a wisdom skill for the given character
    private int rollWisdomCheck(NPC NPC) {
        RollFormula rollFormula = new RollFormula(1, 20, NPC.getAbilityScores().toModifier("wisdom"));
        return rollFormula.roll();
    }

    // EFFECTS: rolls a charisma check for the given character
    private int rollCharismaCheck(NPC NPC) {
        RollFormula rollFormula = new RollFormula(1, 20, NPC.getAbilityScores().toModifier("charisma"));
        return rollFormula.roll();
    }
    ...
    private void displayInitiativeMenu() {
        ...
        if (selectedNPC != null) {
            System.out.println(selectedNPC.getTitle().getName()
                    + "'s initiative: " + rollDexterityCheck(selectedNPC) + ".");
        } else {
            for (NPC c : selectedGroup) {
    ...
    private void createCustomStatBlock() {
        ...
        AbilityScoreSet abilityScoreSet = getCustomStatBlockAbilityScores();

        StatBlock customStatBlock = new StatBlock.StatBlockBuilder(title, xp, hpFormula, proficiency, armour, speeds,
                senses, abilityScoreSet, getCustomStatBlockActions())
    ...
    private AbilityScoreSet getCustomStatBlockAbilityScores() {
        String as = " ability score";

        return new AbilityScoreSet(getCustomStatBlockInteger("strength" + as),
    ...
    private List<RollableAction> getCustomStatBlockActions() {
        ...
        if (numberOfActions == 0) {
            ...
            for (int i = 1; i < numberOfActions; i++) {
                ...
                RollableActions.add(getCustomAction());
            }
        }
    ...
    private RollableAction getCustomAction() {
        ...
        return new RollableAction(name, description, reach, hit, damage);
    }
    ...
    private void createCharacters() {
        ...
        if (numberOfCopies <= 0) {
            ...
            for (int i = 1; i < (numberOfCopies + 1); i++) {
                NPC NPC = new NPC(selectedStatBlock,
                        NPC.generateNameForEncounter(selectedStatBlock, encounter));
    ...
    private void initializeOrcStatBlock() {
        ...
        RollableAction orcGreatAxe = new RollableAction("GreatAxe", "Melee Weapon Attack", "5",
        ...
        RollableAction orcJavelin = new RollableAction("Javelin", "Melee or Ranged Weapon Attack", "5 or 30/120",
                new RollFormula(1, 20, 5),  //hit formula
                javelinDamage); //damage formula

        List<RollableAction> orcRollableActions = new ArrayList<>();
        orcRollableActions.add(orcGreatAxe);
        orcRollableActions.add(orcJavelin);
        ...
        StatBlock orc = new StatBlock.StatBlockBuilder(
                ...
                (new AbilityScoreSet(16, 12, 16, 7, 11, 10)),
                orcRollableActions)
    ...
    private void initializeAncientBlackDragonStatBlock() {
        ...
        RollableAction bite = new RollableAction("Bite", "Melee Weapon Attack",  "15",
        ...
        RollableAction claw = new RollableAction("Claw", "Melee Weapon Attack",  "10",
                new RollFormula(1, 20, 15),  //hit formula
        ...
        RollableAction tail = new RollableAction("Tail", "Melee Weapon Attack", "20",
                new RollFormula(1, 20, 15),  //hit formula
                tailDamage); //damage formula

        List<RollableAction> ancientBlackDragonRollableActions = new ArrayList<>();
        ancientBlackDragonRollableActions.add(bite);
        ancientBlackDragonRollableActions.add(claw);
        ancientBlackDragonRollableActions.add(tail);
        ...
        StatBlock ancientBlackDragon = new StatBlock.StatBlockBuilder(
                ...
                (new AbilityScoreSet(27, 14, 25, 16, 15, 19)),
                ancientBlackDragonRollableActions)
